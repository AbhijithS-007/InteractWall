...
std::vector<char> GL_ReadFileContent(const std::string& filename) {
...
void GL_Initialize(RendererContext* ctx) {
    g_ctx = ctx;
    std::cout << "[GravityLens] Initializing...\n";
    ...
    auto vsData = GL_ReadFileContent(pluginDir + "GL_FullscreenVS.cso");
    auto simData = GL_ReadFileContent(pluginDir + "GL_SimPS.cso");
    auto compData = GL_ReadFileContent(pluginDir + "GL_CompositePS.cso");
...
void GL_Update(float deltaTime) {
    ...
    if (s_debugTimer >= 1.0f) {
        s_debugTimer = 0.0f;
        std::cout << "[GravityLens] LIVE SETTINGS -> "
                  << "Strength: " << g_settings.lensStrength << ", "
                  << "Radius: " << g_settings.lensRadius << ", "
    ...
    if (SUCCEEDED(g_ctx->context->Map(g_SimCB, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped))) {
        ...
        scb->lensRadius = g_settings.lensRadius;
        scb->lensStrength = g_settings.lensStrength;
...
void GL_Render() {
    ...
    if (SUCCEEDED(g_ctx->context->Map(g_CompositeCB, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped))) {
        ...
        ccb->lensRadius = g_settings.lensRadius;
...
void GL_OnWallpaperChanged(const WallpaperLayers* layers) {
    if (g_TexBase) { g_TexBase->Release(); g_TexBase = nullptr; }
    if (g_SRVBase) { g_SRVBase->Release(); g_SRVBase = nullptr; }
...
void GL_OnMonitorChanged(const MonitorInfo* info) {}

void GL_OnQualityTierChanged(const QualityTier* tier) {
...
void GL_LoadSettings(const char* jsonPath) {
    try {
        std::ifstream file(jsonPath);
        if (file.is_open()) {
            json j;
            file >> j;
            if (j.contains("lensStrength")) g_settings.lensStrength = j["lensStrength"];
            if (j.contains("lensRadius")) g_settings.lensRadius = j["lensRadius"];
...
void GL_SaveSettings(const char* jsonPath) {
    try {
        json j;
        j["lensStrength"] = g_settings.lensStrength;
        j["lensRadius"] = g_settings.lensRadius;
...
void GL_OnSettingChanged(const char* key, float value) {
    std::string k(key);
    if (k == "lensStrength") g_settings.lensStrength = value;
    else if (k == "lensRadius") g_settings.lensRadius = value;
...
static IEffectPlugin g_plugin = {
    ...
    GL_Shutdown,
    GL_OnMouseMove,
    GL_OnWallpaperChanged,
    GL_OnMonitorChanged,
    GL_OnQualityTierChanged,
    GL_LoadSettings,
    GL_SaveSettings,
    GL_OnSettingChanged
};
